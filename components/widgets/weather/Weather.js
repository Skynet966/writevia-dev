...
export default class Weather extends React.Component{
    ...
    getWeather(){
        if(this.state.lat&this.state.lon){
            const url = `https://api.openweathermap.org/data/2.5/weather?lat=${this.state.lat}&lon=${this.state.lon}&units=metric&appid=7d0aef1d6da83a6ba6a6cb629993f2f3`;
    ...
    getLocation(){
        ...
        promise.then(response=>{
            ...
            console.log("There is some error in iplocate");
        })
    }
    componentDidMount(){
        this.getLocation();
